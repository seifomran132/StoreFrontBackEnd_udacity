...
export class OrderModel {
    ...
    async createOrder(o: order): Promise<order | string> {
        try {
            const conn: PoolClient = await client.connect() // Start DB Connection
            const sql: string = `INSERT INTO orders(product_id, user_id, quantity, status) VALUES ($1, $2, $3, $4) RETURNING *`;
            const result: QueryResult = await conn.query(sql, [o.product_id, o.user_id, o.quantity, o.status]);
            conn.release() // Release DB Connection
    ...
    async delete(o_id: number): Promise<order | string> {
        try {
          ...
          const sql: string = "delete from orders where orders.id = $1 returning *";
